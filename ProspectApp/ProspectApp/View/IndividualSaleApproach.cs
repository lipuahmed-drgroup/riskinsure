...
namespace ProspectApp.View {
    public partial class IndividualSaleApproach : Form {
        Wrappers.SalesApproach currentSA;
        ProspectApp.Controller.Main main;
        List<RIBroker> rbList;
        int cID;
        private int p;
        private SalesApproachManager.DGListItem dGListItem;
        


        public IndividualSaleApproach() {
            InitializeComponent();
        }

        public IndividualSaleApproach(Controller.Main main, SalesApproachManager.DGListItem dGListItem, decimal p_2) {
            InitializeComponent();
            this.main = main;
            this.dGListItem = dGListItem;
            saYear.Text = p_2.ToString();
            cName.Text = dGListItem.cName;
            currentSA = new Wrappers.SalesApproach(main.GetDataRow("saID", "Prospects_SalesApproach", dGListItem.saID.ToString().Trim()));
            tb_status.Text = (dGListItem.completed ? "Closed Approach" : "Open Approach");
            if(!dGListItem.completed) {
                but_final.Enabled = true;
            }
            cID = dGListItem.cID;
            InitialSetup();
            Display(currentSA);
        ...
        //TODO: check if works
        private void but_final_Click(object sender, EventArgs e) {
            if(dGListItem.rbID == 1) {
                new ProblemDiag("Unable to finalize an unassigned sales approach", "saErr3").ShowDialog();
                return;
            }
            but_final.Enabled = false;
            tb_status.Text = "Closed Approach";
            String sql = "UPDATE Prospects_SalesApproachManagement SET isClosed = 1, LastUpdated = '" + DateTime.Now.ToString("yyyyMMdd", CultureInfo.InvariantCulture) + "' where samID = " + dGListItem.samID + ";";
            dGListItem.completed = true;
            main.ExecuteAdvQuery(sql);
            currentSA.updtDate = DateTime.Now;
            main.UpdateRowToDB(currentSA.GenDataRow(), "Prospects_SalesApproach", "saID", currentSA.saID.ToString().Trim());
            Display(currentSA);
        }
        ...
        private void but_Edit_Click(object sender, EventArgs e) {
            ...
            //}
            if(dGListItem.rbID == 1) {
                new ProblemDiag("Unable to edit an unassigned sales approach", "saErr2").ShowDialog();
                return;
            }
            EnableForm();
        ...
        private void but_update_Click(object sender, EventArgs e) {
            ...
            String sql = "UPDATE Prospects_SalesApproachManagement set LastUpdated = '" + DateTime.Now.ToString("yyyyMMdd", CultureInfo.InvariantCulture) + "' where samID = " + dGListItem.samID + ";";
            main.ExecuteAdvQuery(sql);
            PopData();
            DisableForm();
            ClearForm();
